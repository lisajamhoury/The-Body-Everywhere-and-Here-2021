// The Body Everywhere and Here Class 5: Example — Intersecting Forms
...
// Use lodash to make a true copy of myOrderedPose
...
function setup() {
  ...
  // Everytime we get a pose from posenet, call "gotMyPose"
  // and pass in the results
  poseNet.on('pose', (results) => gotMyPose(results));
}
...
function draw() {
  ...
  if (typeof myPose.pose === 'undefined') {
    console.log('returning! my pose is undefined');
    return;
  }

  // if mirroring, just make a mirror to test interaction
  if (mirror) mirrorPoseAndSkeleton();

  // If partner data is empty
  if (partnerPose.length < 1) {
...
// Runs when we receive a pose from the peer connection
function gotPartnerPose(data, id) { 
  // put partner's pose in a global variable 
